...
/**
 * The pathway — potential in motion.
 *
 * One continuous curve with stops along it. The bend is deliberate: a straight
 * line between six labels is a list, and the whole claim of this product is
 * that the steps lead into one another. Where a stop sits is computed on the
 * curve, so a label never floats away from its node.
 */
export function Flow({
  ...
  stops: string[];
  /** How many stops have been passed. */
  reached?: number;
  className?: string;
}) {
  const width = 1000;
  const height = 150;
  // A shallow S: rises, settles, rises again. Sampled rather than guessed so
  // the nodes sit exactly on the stroke.
  const at = (t: number) => ({
    x: 90 + t * (width - 180),
    y: height / 2 - Math.sin(t * Math.PI * 1.6) * 30 + t * 10,
  });
  ...
  const cut = stops.length > 1 ? Math.max(reached - 1, 0) / (stops.length - 1) : 0;
  const passed = Array.from({ length: 60 }, (_, i) => i / 59)
    .filter((t) => t <= cut)
    .map((t, i) => {
      const { x, y } = at(t);
      return `${i === 0 ? "M" : "L"} ${x.toFixed(1)} ${y.toFixed(1)}`;
  ...
  return (
    ...
    >
      <path d={curve} stroke="currentColor" strokeWidth="1.5" opacity="0.3" />
      {passed && (
        <path d={passed} stroke="currentColor" strokeWidth="2.5" opacity="0.95" />
      )}

      {stops.map((stop, index) => {
        const t = stops.length > 1 ? index / (stops.length - 1) : 0;
        const { x, y } = at(t);
        ...
        return (
          <g key={stop}>
            {current && (
              <circle cx={x} cy={y} r="11" stroke="currentColor" strokeWidth="1" opacity="0.5" />
            )}
            <circle
              cx={x}
              cy={y}
              r={current ? 5.5 : 4}
              fill={done || current ? "currentColor" : "none"}
              stroke="currentColor"
              strokeWidth="1.6"
